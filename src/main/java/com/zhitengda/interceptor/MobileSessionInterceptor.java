...
*
 * @author langao_q
 * @create 2020-07-29 16:51


@Slf4j
public class MobileSessionInterceptor {
...
*
    ...
    static final String oauth_url_getuserinfo_usertoken="https://api.weixin.qq.com/cgi-bin/user/info";
    //static final String oauth_url_getuserinfo_usertoken="https://api.weixin.qq.com/cgi-bin/user/info?access_token=ACCESS_TOKEN&openid=OPENID&lang=zh_CN";
    ...
    public void doIntercept(Invocation ai) {
        ...
        logger.debug("========in===uri:" + uri+"===param=="+request.getQueryString());
        String str_appid=request.getParameter("appid");
String str_appid="";
		String str_paramname=controller.getPara(0);
		if(StringUtils.equals(str_paramname, "appid")){
			str_appid =controller.getPara(1);
		}

        if( StringUtils.isNotBlank(str_appid)){
            if(ai.getController().getSession().getAttribute(Constants.APPID_IN_SESSION)==null){
                ai.getController().getSession().setAttribute(Constants.APPID_IN_SESSION, str_appid);

            }else{
                if(!((String)ai.getController().getSession().getAttribute(Constants.APPID_IN_SESSION)).equals(str_appid)){
                    ai.getController().getSession().setAttribute(Constants.APPID_IN_SESSION, str_appid);
                    ai.getController().getSession().removeAttribute(Constants.BUYER_USER_IN_SESSION);
                    ai.getController().getSession().removeAttribute(Constants.APPUSER_IN_SESSION);
                }
            }

        }else{
            str_appid  ="";
            if(StringUtils.isNotBlank( (String)ai.getController().getSession().getAttribute(Constants.APPID_IN_SESSION))){
                str_appid = (String)ai.getController().getSession().getAttribute(Constants.APPID_IN_SESSION);
            }
        }
...
//        boolean isMobile = userAgent.matches(".*(android|avantgo|bada\\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\\/|plucker|pocket|psp|symbian|treo|up\\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino).*")||userAgent.substring(0,4).matches("1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\\-(n|u)|c55\\/|capi|ccwa|cdm\\-|cell|chtm|cldc|cmd\\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\\-s|devi|dica|dmob|do(c|p)o|ds(12|\\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\\-|_)|g1 u|g560|gene|gf\\-5|g\\-mo|go(\\.w|od)|gr(ad|un)|haie|hcit|hd\\-(m|p|t)|hei\\-|hi(pt|ta)|hp( i|ip)|hs\\-c|ht(c(\\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\\-(20|go|ma)|i230|iac( |\\-|\\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\\/)|klon|kpt |kwc\\-|kyo(c|k)|le(no|xi)|lg( g|\\/(k|l|u)|50|54|e\\-|e\\/|\\-[a-w])|libw|lynx|m1\\-w|m3ga|m50\\/|ma(te|ui|xo)|mc(01|21|ca)|m\\-cr|me(di|rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\\-2|po(ck|rt|se)|prox|psio|pt\\-g|qa\\-a|qc(07|12|21|32|60|\\-[2-7]|i\\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\\-|oo|p\\-)|sdk\\/|se(c(\\-|0|1)|47|mc|nd|ri)|sgh\\-|shar|sie(\\-|m)|sk\\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\\-|v\\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\\-|tdg\\-|tel(i|m)|tim\\-|t\\-mo|to(pl|sh)|ts(70|m\\-|m3|m5)|tx\\-9|up(\\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|xda(\\-|2|g)|yas\\-|your|zeto|zte\\-");
        ...
        if(PropKit.getBoolean("jfinal.devmode") && !PropKit.getBoolean("needAuth")){
            ...
            str_appid =WxSdkPropKit.get("wx_app_id");
            ...
            if(ai.getController().getSession().getAttribute(Constants.BUYER_USER_IN_SESSION) == null){
                ...
                if(authUser == null){
                    ...
                    ai.getController().getSession().setAttribute(Constants.APPUSER_IN_SESSION, authUser);
                }
            }
        }
        //处理授权回调的情况不检查session
        if(StringUtils.isBlank(str_appid)){
            controller.redirect("/reauth");
        }
        //else if (!"/callback".equals(uri)) {   2017-12-13 增加目录把商城调换到 mall 下
        else if (!(request.getContextPath()+"/callback").equals(uri)) {
            //checkSession(ai, WxSdkPropKit.get("wx_app_id"));
            checkSession(ai, str_appid);
        }else {
            ...
            if (StringUtils.isNotBlank(error)) {
                ...
                String result = HttpKit.post(oauth_url_code, genOauthMap(code, getRedirectUrl(request, rUrl),str_appid), "");
...
//                String scope = jsonObj.getStr("scope");
   ...
   String userResult = HttpKit.get(oauth_url_getuserinfo_usertoken,
        				ParaMap.create("access_token", accessToken).put("openid", openId).put("lang", "zh_CN").getData());
                ...
                BuyerUser member = updateUserInfo(userInfo, ai,str_appid);
                ...
                if(authUser == null){
                    //authUser = AuthUser.dao.findFirst("select * from " + AuthUser.table + " where app_id=?", WxSdkPropKit.get("wx_app_id"));
                    authUser = AuthUser.dao.findFirst("select * from " + AuthUser.table + " where app_id=?", str_appid);
                    //request.getSession().setAttribute(Constants.APPUSER_IN_SESSION, authUser);
                }
                request.getSession().setAttribute(Constants.BUYER_USER_IN_SESSION, member);
                request.getSession().setAttribute(Constants.APPUSER_IN_SESSION, authUser);
                ...
                //----------没有关注公众号就关注公众号-----------------
  if(member.getSubscribe()==0){
                	controller.redirect(authUser.getTransurl());
                }else{
                //---------------------------------------
        		 request.getSession().setAttribute(Constants.BUYER_USER_IN_SESSION, member);
        		 request.getSession().setAttribute(Constants.APPUSER_IN_SESSION, authUser);
    ...
    protected static String getRedirectUrl(HttpServletRequest request, String rUrl) {
        String mainUrl = "http://" + request.getServerName()+ request.getContextPath();//微信只支持80端口  增加 request.getContextPath()  20171213
        //String mainUrl = "http://" + request.getServerName();//+ request.getContextPath();//微信只支持80端口  增加 request.getContextPath()
...
*
    ...
    private void gotoOauth(Controller controller, String appKey, String scope) throws IOException {
        //2017-11-28 增加信息  20171213
        //String mainUrl = "http://" + controller.getRequest().getServerName();//+ controller.getRequest().getContextPath();
        String mainUrl = "http://" + controller.getRequest().getServerName()+controller.getRequest().getContextPath();//+ controller.getRequest().getContextPath();
    ...
    public Map<String, String> genOauthMap(String code, String rUrl,String appid) {
        //根据appid 获取 微信信息
        AuthUser authUser = AuthUser.dao.findFirst("select * from " + AuthUser.table + " where app_id = ? and active=1", appid);
        Map<String, String> props = new HashMap<String, String>();
        props.put("code", code);
        //props.put("appid", WxSdkPropKit.get("wx_app_id"));
        //props.put("secret", WxSdkPropKit.get("wx_app_secret"));
        props.put("appid", authUser.getAppId());
        props.put("secret", authUser.getAppSec());
    ...
    private BuyerUser updateUserInfo(ApiResult userInfo, Invocation ai,String appid) {
        Controller controller = ai.getController();
